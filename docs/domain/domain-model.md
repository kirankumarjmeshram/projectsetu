...
All ratio arithmetic, aggregation, reconciliation, division, and percentage conversion uses `ProjectSetuDecimal` at configured precision with no intermediate rounding. Presentation rounding belongs to report/UI layers. The engine does not recalculate revenue, operating expenses, EBITDA, EBIT, PAT, depreciation, interest, principal, debt balances, balance-sheet values, or project cost. It contains no approval threshold, lender policy, PMEGP/NLM/PMFME rule, subsidy calculation, UI, persistence, report rendering, or investment-cash-flow reconstruction. Investment returns are owned separately by the normalized cash-flow-series module. Lender-specific formulas and thresholds, CMA ratios, working-capital bank-finance methods, credit scoring, collateral ratios, drawing power, and industry benchmarks remain deferred.

## Investment returns and viability metrics

`InvestmentCashFlowSeries` is the sole calculation boundary for Task 012. It carries project identity, an explicit `PROJECT_RETURN` or `EQUITY_RETURN` perspective, and ordered equally spaced annual periods `0..N`. Period 0 is the investment point; period 1 is the end of Year 1. Every period contains an authoritative signed net investment `cashFlow`. The module never substitutes PAT, EBITDA, EBIT, cumulative accounting profit, closing cash, balance-sheet balances, DSCR, or loan outstanding for this series. Date-based XIRR/XNPV and irregular periods remain deferred.

Project-return flows measure resources deployed and cash generated by the underlying project independently of financing. `composeProjectInvestmentCashFlowSeries` accepts source-backed initial investment, normalized operating project cash flow, working-capital investment, later capex, optional explicitly source-backed terminal value, optional explicitly source-backed working-capital recovery, and other explicit project cash flow. It calculates `project cash flow = operating cash flow + terminal value + working-capital recovery + other explicit flow - initial investment - working-capital investment - capex`. An omitted optional recovery contributes zero; it is not estimated or represented as a recovered amount. Initial investment is a non-negative magnitude restricted to period 0; later investments use capex or working capital. Loan disbursement, promoter contribution, principal repayment, cash interest, PAT, EBITDA, EBIT, closing cash, and financing-inclusive net cash movement are forbidden runtime and contract fields. The existing `CashFlowSchedule.netCashMovement` is therefore never a Project IRR adapter source.

Equity-return calculations are supported only for an already-authoritative `EQUITY_RETURN` series. ProjectSetu does not yet compose that series because promoter contributions, investor distributions, and residual equity value lack a complete authoritative distribution model. It never treats all financing cash flows as equity cash flows or mixes project and equity perspectives.

NPV uses `NPV = sum(CF_t / (1 + r)^t)`. Period 0 has discount factor 1 and is never discounted. The source-backed per-period discount rate may be zero or positive; Task 012 rejects negative discount rates and never infers a rate from loan interest, inflation, a scheme, WACC, or CAPM. `DiscountedCashFlowRow` exposes the multiplier discount factor, present value, and exact cumulative present value without presentation rounding.

IRR is the periodic rate where the Decimal.js-evaluated NPV is zero. Zeros are ignored when counting sign changes. No sign change yields `UNDEFINED_NO_SIGN_CHANGE`. More than one sign change can permit multiple IRRs but does not prove that multiple roots exist; Task 012 conservatively yields `AMBIGUOUS_MULTIPLE_IRR_POSSIBLE` rather than searching for and presenting one root as uniquely authoritative. Exactly one sign change first checks an exact 0% root, then uses a lower bound just above −100%, an initial 100% upper bound, deterministic upper-bound expansion, and bisection. Exact roots on initial or expanded boundaries are accepted. The default policy caps expansion at 64 steps and bisection at 256 iterations, with explicit Decimal rate and residual-NPV tolerances. A rate is `DEFINED` only when its Decimal NPV residual is within the configured NPV tolerance; exhaustion of the rate interval, bracket, or iteration limit without that residual yields `NUMERICAL_CONVERGENCE_FAILURE`. Iteration counters use native integers only for loop control; every financial rate bound, midpoint, NPV, cash flow, power, comparison, and residual uses `ProjectSetuDecimal`. The search domain remains strictly above −100%.

Simple payback accumulates undiscounted cash flows. When negative cumulative investment is first recovered in period `t`, `payback = (t - 1) + unrecovered amount at period start / recovery-period inflow`. Discounted payback applies the same interpolation to Decimal.js present values. If cumulative cash flow never becomes non-negative, the result is `NOT_RECOVERED_WITHIN_HORIZON`. If cumulative cash flow is never negative, including zero initial investment with no later deficit, payback is defined as zero. Multiple negative investment periods before recovery are supported by cumulative continuity. Under the conventional payback definition, a later non-conventional negative cash flow does not revoke an earlier first-recovery result; post-recovery cash-flow risk must be evaluated separately.

Profitability index uses the conventional single-initial-investment formula `PI = PV of future positive cash flows / absolute period-0 investment`. Zero initial investment yields `UNDEFINED_ZERO_INITIAL_INVESTMENT`. A positive period-0 flow or any later negative cash flow yields `INVALID_CASH_FLOW_PATTERN`; the engine does not silently apply a simple PI formula to multi-investment structures. NPV itself remains valid for any structurally valid signed series.

`InvestmentReturnMetric` provides deterministic `DEFINED`, no-sign-change, multiple-IRR ambiguity, not-recovered, zero-initial-investment, invalid-pattern, and numerical-convergence states. Undefined outputs have no value and never contain zero substitutes, `NaN`, or `Infinity`. Weak projects may have negative NPV, negative IRR, long/no payback, or PI below one without becoming invalid and without receiving GOOD/BAD, viable/non-viable, bankable, or approval labels.

Terminal value and working-capital recovery enter only through explicitly supplied source-backed components. Omission means no recovery cash flow. Neither is inferred from depreciation residual value, book value, accumulated depreciation, final working-capital balance, project cost, promoter contribution, or bank finance. All NPV, present value, cumulative value, payback interpolation, PI, IRR NPV evaluation, and result serialization use Decimal.js with no intermediate financial rounding.

MIRR, XIRR, XNPV, irregular/monthly/date-based returns, WACC, CAPM, automatic discount-rate inference, lender hurdle rates, scheme viability rules, automatic terminal/asset-sale estimation, automatic working-capital recovery, disposal tax, Monte Carlo/stochastic returns, scenario expansion, inflation modelling, and a generated equity-return adapter remain deferred. The module contains no PMEGP/NLM/PMFME, subsidy-rule, UI, database, or report-rendering logic.
...
The former placeholder metric result contracts have been superseded by the dedicated `BankabilityMetricsSchedule`, `InvestmentReturnsAnalysis`, and explicit metric-status models. MIRR and irregular-date investment-return variants remain deferred.
