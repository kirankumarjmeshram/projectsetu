      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-xs md:p-8">
        ...
        {currentStep === 8 && (
          <Step8Schemes
            ...
            onFactsChange={(schemeFacts) => setInput({ ...input, schemeFacts })}
          />
        )}
        ...
        <div className="mt-8 flex items-center justify-between border-t border-slate-100 pt-8">
          <button
            ...
            className="rounded-lg border border-slate-300 px-5 py-2 text-xs font-semibold text-slate-700 transition-colors hover:bg-slate-50 disabled:opacity-30"
          >
            ← Back
          </button>
          ...
          {currentStep < 9 ? (
            <button
              type="button"
              onClick={handleNext}
              className="rounded-lg bg-emerald-700 px-6 py-2 text-xs font-bold text-white shadow-xs transition-colors hover:bg-emerald-800"
            >
            ...
            <button
              ...
              className="rounded-lg bg-emerald-700 px-6 py-2 text-xs font-bold text-white shadow-xs transition-colors hover:bg-emerald-800"
            >
              {isPending ? "Calculating..." : "Execute & View Statements →"}
            </button>
          ) : (
            <button
              type="button"
              onClick={() => handleStepChange(9)}
              className="rounded-lg border border-slate-300 px-5 py-2 text-xs font-semibold text-slate-700 transition-colors hover:bg-slate-50"
            >
              ← Edit Assumptions
            </button>
          )}
